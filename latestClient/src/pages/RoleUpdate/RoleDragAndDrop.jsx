...
const RoleDragAndDrop = () => {
  ...
  const fetchRoles = async () => {
    try {
      const notRolesResponse = await fetch(`/api/notroles/${user.USER_ID}`);
      const assignedRolesResponse = await fetch(
        `/api/roleassigned/${user.USER_ID}`
      );
      let notRolesData = [];
  ...
  const handleOnDropLeft = async (event) => {
    ...
    if (item) {
      try {
        ...
        await axios.delete(`/api/deleteroles/${user.USER_ID}/${item.ROLE_ID}`);
    ...
    <div className="mt-20">
      <div className="grid grid-cols-3 g-4 m-4 justify-between card ">
        <div className="text-center m-2 border-dashed border-2  justify-center rounded-lg shadow-lg p-4">
          ...
          <div className="text-sm">
            <Link
              to="/addRole"
              className=" text-white mx-auto text-sm btn btn-sm btn-primary "
            >
              Back
            </Link>
          </div>
      ...
      <div>
        ...
        <div className="grid grid-cols-2 gap-3 p-4 m-4">
          ...
          >
            {/* Source List */}
            <p className="text-center m-2 text-sm text-white mx-auto btn btn-sm btn-success">
              Roles{" "}
            </p>
